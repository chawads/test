(function($){
        $.fn.extend({
            tableExport: function($aid, options) {
                var defaults = {
						separator: ',',
						ignoreColumn: [],
				...
				if(defaults.type == 'csv' || defaults.type == 'txt'){
					...
					csvData = 'data:application/csv;charset=utf-8,' + encodeURIComponent(tdData);
					$($aid).attr({
						'download': defaults.tableName.toString() + '.'+defaults.type,
						'href': csvData,
						'target': '_blank'
					});
					...
					doc.save(defaults.tableName.toString() + '.'+defaults.type);		
				}
